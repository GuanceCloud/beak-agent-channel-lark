...
飞书/Lark WebSocket 连接归属仍在 Beak host。SDK 不持有 WebSocket client、heartbeat 或 reconnect loop；host WebSocket runtime 负责写 connection/reconnect health，SDK 只写事件 activity 时间。
